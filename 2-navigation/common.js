module.exports = {
    move: {
        ...
        up (position, distance)
        {
            position.Vertical -= distance;
        },
    
        down (position, distance)
        {
            position.Vertical += distance;
        }
    },
    ...
    processRawData: (raw) => {
        ...
        {
            let command = arr[i];
            let distance = parseInt(arr[i+1]);
            let entry = {command, distance};
